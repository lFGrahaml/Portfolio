    ...
    <div className='fixed top-0 right-0 w-11/12 -z-10 translate-y-[-80%]'>
      ...
      <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? 'bg-white bg-opacity-50 backdrop-blur-lg shadow-sm' : ''}`}>
        ...
        <div className='flex items-center gap-4'>
          ...
          <button>
            <Image src="/assets/moon_icon.png" alt='' width={20} height={20} className='w-6'/>
          </button>

          <a href="#contact" className='font-display hidden lg:flex items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4'>Contact <Image src="/assets/arrow-icon.png" alt="Arrow icon" width={39} height={39} className='w-3'/> </a>

          <button className='block md:hidden ml-3 cursor-pointer' onClick={openSideBar}>
            <Image src="/assets/menu-black.png" alt='' width={20} height={20} className='w-6'/>
          </button>
        </div>

        {/* Sidebar */}

        <ul className={`flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 ${isOpen ? 'translate-x-[-16rem]' : 'translate-x-[16rem]'}`}>

          <div className='absolute right-6 top-6' onClick={closeSideBar}>
            <Image src="/assets/close-black.png" alt='' width={20} height={20} className='w-5 cursor-pointer'/>
          </div>
